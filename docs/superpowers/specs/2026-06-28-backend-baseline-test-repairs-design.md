...
Change the API test fixture to yield a context-managed `TestClient(app)`. Entering the context runs the real application lifespan, including database schema creation, and exiting it performs the matching shutdown. Production database configuration and application startup code remain unchanged.
...
Remove the blanket `:has(...)` early-return guard and allow `soup.select()` to evaluate these selectors through SoupSieve. Retain the existing selector exception handling so an unsupported or malformed selector produces no crash and is logged at debug level.

Add a focused unit regression test that constructs a selector check containing `:has(...)`, verifies the selector is evaluated, and confirms the expected accessibility issue is returned. The existing `test_table_without_headers` remains the higher-level integration regression.
...
2. `_check_selector` passes the complete selector to `soup.select()` even when it contains `:has(...)`.
3. Matching elements become normal `AccessibilityIssue` objects through the existing mapping path.
4. Selector evaluation exceptions continue to be caught by the existing handler.
...
- Add a focused failing `:has(...)` selector regression test before removing the guard.
- Verify the focused selector test and `TestHTMLAnalysis::test_table_without_headers` pass after the change.
...
- Implementing a custom CSS selector parser.
